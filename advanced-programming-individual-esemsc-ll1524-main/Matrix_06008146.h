...
#ifndef _ADV_PROG_MATRIX_06008146_H_
#define _ADV_PROG_MATRIX_06008146_H_
...
namespace adv_prog_cw 
{
	...
	class Matrix_06008146 {
	public:
		Matrix_06008146();
		Matrix_06008146(size_t m, size_t n);
		Matrix_06008146(size_t m, size_t n, fT val);
		Matrix_06008146(const Matrix_06008146& M);
		~Matrix_06008146();
		...
		// assignment
		Matrix_06008146& operator=(const Matrix_06008146& M);
		Matrix_06008146& operator=(fT val);

		Matrix_06008146& operator+=(const Matrix_06008146& M);
		Matrix_06008146& operator-=(const Matrix_06008146& M);
		void Identity();
		void Zero();
		void Transposed(Matrix_06008146& RES) const;
		...
		Matrix_06008146& operator*=(fT scalar);
		...
		bool Inverse(Matrix_06008146& result) const;
	...
	private:
		...
		bool  CheckSizes(const Matrix_06008146& mat, const char* originator) const;
	};

	// associated operators
	template<typename fT>
	Matrix_06008146<fT>  operator+(const Matrix_06008146<fT>& a, const Matrix_06008146<fT>& b);
	...
	template<typename fT>
	std::vector<fT>  operator*(const Matrix_06008146<fT>& mat, const std::vector<fT>& v);

	// v^T = (v^T M)^T
	template<typename fT>
	Matrix_06008146<fT>  operator*(const std::vector<fT>& v, const Matrix_06008146<fT>& M);
	...
	//
	template<typename fT>
	inline Matrix_06008146<fT>::Matrix_06008146(size_t m, size_t n)
	...
	//
	template<typename fT>
	inline fT& Matrix_06008146<fT>::operator()(size_t m, size_t n)
	{
#ifndef NDEBUG
		CheckRange(m, n, "Matrix_06008146<fT>::operator()");
#endif
		return data[m][n];
	}
	...
	//
	template<typename fT>
	inline const fT& Matrix_06008146<fT>::operator()(size_t m, size_t n) const
	{
#ifndef NDEBUG
		CheckRange(m, n, "Matrix_06008146<fT>::operator()");
#endif
	...
	//
	template<typename fT>
	inline size_t Matrix_06008146<fT>::Rows() const { return rows; }

	// Cols()
	// ---------------------------------------------
	//
	template<typename fT>
	inline size_t Matrix_06008146<fT>::Cols() const { return cols; }

	// CheckRange(i,j, message )
	// ---------------------------------------
	//
	template<typename fT>
	inline bool Matrix_06008146<fT>::CheckRange(size_t m, size_t n,
	...
	// ---------------------------------------
	template<typename fT>
	inline bool Matrix_06008146<fT>::CheckSizes(const Matrix_06008146& mat,
		const char* originator) const
	{
		if (rows != mat.rows) {
