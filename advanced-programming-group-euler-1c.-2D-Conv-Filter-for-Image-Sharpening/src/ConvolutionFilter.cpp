...
/**
 * @brief Utility function to apply a 2D convolution kernel to a grayscale image
 * 
 * This method performs convolution on a grayscale image using the specified kernel 
 * and normalizes the resulting values to the range [0, 255].
 * 
 * @param input The input grayscale Image on which convolution will be performed
 * @param kernel A 2D convolution kernel represented by a std::vector<std::vector<int>>
 * @return Image A new Image object containing the convolution results (grayscale)
 */
Image ConvolutionFilter::applyConvolution(const Image& input, const std::vector<std::vector<int>>& kernel) {
    int width = input.getWidth();
    int height = input.getHeight();
    Image output(width, height, 1);  // Output as a grayscale image
        ...
        for (int x = 0; x < width; ++x) {
            ...
            for (int ky = 0; ky < static_cast<int>(kernel.size()); ++ky) {
                for (int kx = 0; kx < static_cast<int>(kernel[ky].size()); ++kx) {
                    ...
                    // Get the pixel intensity (assuming grayscale)
